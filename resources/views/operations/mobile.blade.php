...
══════════════════════════════════════════════ */
    [data-theme="light"] {
      ...
      --glass: rgba(255, 248, 245, .96);
      --appbar: #fff8f5;
      --appbar-border: rgba(232, 213, 196, .8);
    }
    ...
    [data-theme="dark"] {
      ...
      --glass: rgba(35, 21, 25, .97);
      --appbar: #231519;
      --appbar-border: rgba(74, 46, 54, .9);
    }
    ...
    *::after {
    ...
    .bg-orb-1 {
      width: 300px;
      height: 300px;
      background: radial-gradient(circle, rgba(201, 99, 122, .22), transparent 70%);
      top: -60px;
      right: -60px;
    ...
    .bg-orb-2 {
      width: 220px;
      height: 220px;
      background: radial-gradient(circle, rgba(176, 112, 96, .16), transparent 70%);
      bottom: 120px;
      left: -60px;
    ...
    .bg-orb-3 {
      width: 180px;
      height: 180px;
      background: radial-gradient(circle, rgba(255, 194, 205, .18), transparent 70%);
      top: 45%;
      left: 40%;
    ...
    [data-theme="dark"] .bg-orb-1 {
      background: radial-gradient(circle, rgba(255, 143, 163, .18), transparent 70%);
    }

    [data-theme="dark"] .bg-orb-2 {
      background: radial-gradient(circle, rgba(212, 144, 122, .14), transparent 70%);
    }

    [data-theme="dark"] .bg-orb-3 {
      background: radial-gradient(circle, rgba(255, 143, 163, .12), transparent 70%);
    }
    ...
    @keyframes floatOrb2 {
      ...
      66% {
        transform: translate(16px, -22px) scale(.93);
        opacity: .22;
      }
    }
...
══════════════════════════════════════════════ */
    ...
    .card-accent-bar.s-done {
      background: linear-gradient(90deg, var(--done), rgba(90, 154, 106, .3));
    }

    .card-accent-bar.s-onhold {
      background: linear-gradient(90deg, var(--onhold), rgba(176, 128, 32, .3));
    }

    .card-accent-bar.s-revision {
      background: linear-gradient(90deg, var(--revision), rgba(201, 96, 112, .3));
    }
    ...
    .s-done {
      ...
      color: var(--done);
      border: 1.5px solid rgba(90, 154, 106, .25);
    }
    ...
    .s-onhold {
      ...
      color: var(--onhold);
      border: 1.5px solid rgba(176, 128, 32, .25);
    }
    ...
    .s-revision {
      ...
      color: var(--revision);
      border: 1.5px solid rgba(201, 96, 112, .25);
    }
    ...
    .stage-step.done .stage-step-icon {
      background: rgba(90, 154, 106, .18);
      color: var(--done);
    }

    .stage-step.active .stage-step-icon {
      background: rgba(201, 99, 122, .2);
      color: var(--accent);
    }

    .stage-step.pending .stage-step-icon {
      background: var(--border);
      color: var(--muted);
    }
    ...
    .due-notif.urgent {
      background: rgba(201, 96, 112, .08);
      color: var(--revision);
      border: 1px solid rgba(201, 96, 112, .2);
    }

    .due-notif.warning {
      background: rgba(176, 128, 32, .08);
      color: var(--onhold);
      border: 1px solid rgba(176, 128, 32, .2);
    }

    .due-notif.safe {
      background: rgba(90, 154, 106, .08);
      color: var(--done);
      border: 1px solid rgba(90, 154, 106, .2);
    }
...
══════════════════════════════════════════════ */
    ...
    .overdue-popup::-webkit-scrollbar {
      display: none;
    }
...
<body>
  ...
  <div class="app-bar">
    ...
    <div class="app-bar-right">
      <div class="export-pill-wrap">
        <button class="icon-btn" onclick="toggleExportDrop(event)" title="Export">
  ...
  <div class="container">
    ...
    <!-- ══ TOOL STRIP ══ -->
    <!-- Hidden badge anchors kept so JS badge logic still works -->
    <span id="log-badge-count" style="display:none">0</span>
    <span id="bin-badge-count" style="display:none">0</span>
    <span id="archive-badge-count" style="display:none">0</span>

    <!-- ══ STATS ROW ══ -->
    <div class="stats-row">
      <div class="stat-card"><span class="stat-val" style="color:var(--done)" id="cnt-done">0</span><span
          class="stat-label">Done</span></div>
      <div class="stat-card"><span class="stat-val" style="color:var(--onhold)" id="cnt-hold">0</span><span
          class="stat-label">On Hold</span></div>
      <div class="stat-card"><span class="stat-val" style="color:var(--revision)" id="cnt-rev">0</span><span
          class="stat-label">Revisions</span></div>
    </div>
    ...
    <div class="filter-section">
      <div class="search-wrap">
        ...
        <input type="text" id="search-input" placeholder="Search clients…" class="search-input"
  ...
  <!-- ══ FAB — Add Client ══ -->
  <button class="fab" id="fab-btn" onclick="openAddModal()" title="Add Client">
    ...
    <div class="modal-box">
      ...
      <div class="form-group">
        ...
        <div class="autocomplete-wrap">
          <input type="text" id="f-prop-assign" class="mobile-input" placeholder="Type a name…" autocomplete="off"
            oninput="acInput('f-prop-assign','ac-prop-assign','all')"
            onfocus="acInput('f-prop-assign','ac-prop-assign','all')"
            onblur="setTimeout(()=>acClose('ac-prop-assign'),150)">
          <div class="ac-dropdown" id="ac-prop-assign"></div>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <textarea id="f-prop-remark" class="mobile-input" rows="2"
          placeholder="Notes about this proposal..."></textarea>
      </div>
      <div class="form-group">
        <label>Final Remarks</label>
        <textarea id="f-final-remark" class="mobile-input" rows="2"
          placeholder="Closing notes, delivery status..."></textarea>
      </div>
  ...
  <div class="card-actions">
    ...
    /* FAB visibility — only show on Home tab */
    (function () {
      function syncFab() {
        const fab = document.getElementById('fab-btn');
        if (!fab) return;
        fab.style.display = activeMobileTab === 'home' ? 'flex' : 'none';
      }
      document.querySelectorAll('.nav-tab').forEach(btn => {
        btn.addEventListener('click', () => setTimeout(syncFab, 0));
      });
      syncFab();
    })();
  </script>
...
════════════════════════════════════════════════ */
    ...
    function acInput(inputId, dropId, group) {
      ...
      const roleLabel = group === 'uiux' ? 'UI/UX Designer' : group === 'dev' ? 'Developer' : 'Team Member';
    ...
    <div class="ac-option" onmousedown="acPick('${inputId}','${dropId}','${n.name}')">
      ...
      <div>
        <div>${n.name}</div>
        <div class="ac-role">${group === 'all' ? (AC_NAMES.uiux.find(x => x.name === n.name) ? 'UI/UX Designer' : 'Developer') : roleLabel}</div>
      </div>
    </div>
  `).join('');
